...
def tela_de_insercao(tela_anterior, janela_principal, migracao,sistema_origem,sistema_destino):
    ...
    button_selecionar = tk.Button(frame_superior, text="Selecionar Arquivo", command=lambda:[lambda:[selecionar_arquivo(var_extensoes.get()),inserir_no_banco(tela_atual,janela_principal,migracao,sistema_origem,sistema_destino,var_extensoes.get())] if (var_extensoes.get() !='Nenhuma opção selecionada') else messagebox.showerror("Erro", f"Selecione uma extensão antes de prosseguir.")])
